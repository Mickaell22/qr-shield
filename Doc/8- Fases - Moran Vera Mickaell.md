# 8- Fases - Moran Vera Mickaell

## Hoja: Portada

|  | Asignatura: |  |
| --- | --- | --- |
|  |  | Gestión de Proyecto |
|  | Curso: |  |
|  | SOF-S-NO-9-2 |  |
|  | Tema: |  |
| Sistema inteligente para la detección de suplantación |  |  |
| en códigos QR mediante análisis en cascada con |  |  |
| trazabilidad de redirecciones aplicable a diversos entornos |  |  |
|  | Integrantes: |  |
|  | Moran Vera Mickael Adrian |  |
|  | Docente: |  |
|  |  | M.Sc. Ángela Yanza |
|  | Año Lectivo: |  |
|  |  | 2026-2027 |
|  |  | Ciclo I |

## Hoja: Menú Principal

|  |  |  | Menú - Fases |
| --- | --- | --- | --- |

## Hoja: 1. DCP

|  |  |  | Metodología de Marco Lógico |  |  |  |  |  |  |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
|  |  |  | Fase 1. Análisis del Problema Central |  |  |  |  |  |  |
|  |  |  |  | Proyecto: | Sistema inteligente para la detección de suplantación en códigos QR mediante análisis en cascada con trazabilidad de redirecciones aplicable a diversos entornos |  |  |  |  |
|  |  |  |  |  |  | 1 | Descripción del Problema Central |  |  |
|  |  |  |  |  |  |  | 1.1 | Análisis Causal |  |
|  |  |  |  |  |  |  |  | 1.2 | Diagrama Ishikawa |

## Hoja: 1.1

|  |  | Análisis del Problema Central |  |  |  |  |
| --- | --- | --- | --- | --- | --- | --- |
|  |  |  | Proyecto: Sistema inteligente para la detección de suplantación en códigos QR mediante análisis en cascada con trazabilidad de redirecciones aplicable a diversos entornos |  |  |  |
|  |  | 1. | Descripción del problema central |  |  |  |
|  |  | 1.1 | Análisis Causal |  |  |  |
|  |  |  |  | N° | Causas | Consecuencias |
|  |  |  |  | 1 | El contenido de un código QR permanece oculto hasta ser escaneado | El usuario accede al destino sin poder evaluar el riesgo previamente |
|  |  |  |  | 2 | Confianza ciega del usuario al escanear códigos QR | Mayor probabilidad de caer en sitios fraudulentos por desconocimiento |
|  |  |  |  | 3 | Los filtros de seguridad tradicionales interpretan el QR como una simple imagen | Los ataques de quishing evaden las defensas convencionales de correo y navegación |
|  |  |  |  | 4 | Facilidad para alterar o suplantar un código QR físico (adhesivos sobre el original) | El usuario es redirigido a destinos maliciosos sin advertirlo |
|  |  |  |  | 5 | Ausencia de una capa intermedia entre el escaneo y el acceso al destino | No existe ninguna barrera de protección antes de visitar el sitio |
|  |  |  |  | 6 | Inexistencia de herramientas locales accesibles para verificar un código QR | El usuario queda sin alternativas de protección integradas a su uso cotidiano |
|  |  |  |  | 7 | Alta exposición a códigos QR en espacios físicos abiertos del campus | Aumento de la superficie de ataque dentro de la comunidad universitaria |
|  |  |  |  | 8 | Perfeccionamiento continuo de las técnicas de ingeniería social | Ataques de quishing cada vez más difíciles de identificar por el usuario |
|  |  |  |  | 9 | Desconocimiento general sobre la existencia del quishing como amenaza | La víctima no toma precauciones por ignorar el riesgo |
|  |  |  |  | 10 | Uso de URL acortadas o enmascaradas dentro de los códigos QR | Dificultad para reconocer el destino real antes de acceder a él |

## Hoja: 1.2

|  |  |  |  | Metodología de Marco Lógico |
| --- | --- | --- | --- | --- |
|  |  |  |  | Fase 1. Descripciòn del Problema Central |
|  |  |  |  | Proyecto:   Sistema inteligente para la detección de suplantación en códigos QR mediante análisis en cascada con trazabilidad de redirecciones aplicable a diversos entornos |
|  |  | 1. | Descripción del problema central |  |
|  |  | 1.2 | Diagrama de Ishikawa |  |
| Ventajas |  |  |  |  |
| Desventajas |  |  |  |  |

## Hoja: 2. AI

|  |  | Metodología de Marco Lógico |  |  |  |  |  |  |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
|  |  | Fase 2. Análisis de Involucrados |  |  |  |  |  |  |
|  |  | Proyecto: | Sistema inteligente para la detección de suplantación en códigos QR mediante análisis en cascada con trazabilidad de redirecciones aplicable a diversos entornos |  |  |  |  |  |
|  |  |  | 2.1 | Matriz de Análisis de Involucrados |  |  |  |  |
|  |  |  |  | 2.1.1 | Matriz Poder - Interés |  |  |  |
|  |  |  |  |  | 2.1.2 | Matriz Influencia - Impacto |  |  |
|  |  |  |  |  |  | 2.1.3 | Matriz Poder - Influencia |  |
|  |  |  |  |  |  | 2.2 | Mapa de Actores |  |
|  |  |  |  |  |  |  | 2.2.1 | Mapas de Actores |

## Hoja: 2.1.1

| Analisis de Involucrados |  |  |  |  |
| --- | --- | --- | --- | --- |
|  |  | Proyecto:   Sistema inteligente para la detección de suplantación en códigos QR mediante análisis en cascada con trazabilidad de redirecciones aplicable a diversos entornos |  |  |
|  | 2.1. | Matriz de Analisís de Involucrados |  |  |
|  | 2.1.1 | Matriz Poder-Interés |  |  |
|  |  | N° | Involucrados | Tipos |
|  |  | 1 | Ing. Msc. Ángela Yanza Montalván | Directo |
|  |  | 2 | Morán Vera Mickaell Adrián (Autor) | Directo |
|  |  | 3 | Universidad de Guayaquil (Autoridades) | Directo |
|  |  | 4 | Carrera de Software – FCMF | Directo |
|  |  | 5 | Estudiantes de la UG (población de estudio) | Directo |
|  |  | 6 | Usuarios de Guayaquil expuestos al quishing | Indirecto |
|  |  | 7 | Comerciantes y PYMES que usan códigos QR | Indirecto |
|  |  | 8 | Entidades financieras con canales QR | Indirecto |
|  |  | 9 | Futuros proyectos de investigación en ciberseguridad | Indirecto |
|  |  | Poder |  |  |
|  |  |  | `-                                 Interés | + |

## Hoja: 2.1.2

|  | Analisis de Involucrados |  |  |  |
| --- | --- | --- | --- | --- |
|  |  | Proyecto:    Sistema inteligente para la detección de suplantación en códigos QR mediante análisis en cascada con trazabilidad de redirecciones aplicable a diversos entornos |  |  |
|  | 2.1. | Matriz de Analisís de Involucrados |  |  |
|  | 2.1.2 | Matriz Influencia-Impacto |  |  |
|  |  | N° | Involucrados | Tipos |
|  |  | 1 | Msc. Ángela Yanza Montalván | Directo |
|  |  | 2 | Morán Vera Mickaell Adrián (Autor) | Directo |
|  |  | 3 | Universidad de Guayaquil (Autoridades) | Directo |
|  |  | 4 | Carrera de Software – FCMF | Directo |
|  |  | 5 | Estudiantes de la UG (población de estudio) | Directo |
|  |  | 6 | Usuarios de Guayaquil expuestos al quishing | Indirecto |
|  |  | 7 | Comerciantes y PYMES que usan códigos QR | Indirecto |
|  |  | 8 | Entidades financieras con canales QR | Indirecto |
|  |  | 9 | Futuros proyectos de investigación en ciberseguridad | Indirecto |
|  |  | Impacto |  |  |
|  |  |  | Influencia |  |

## Hoja: 2.1.3

|  | Analisis de Involucrados |  |  |  |
| --- | --- | --- | --- | --- |
|  |  | Proyecto:   Sistema inteligente para la detección de suplantación en códigos QR mediante análisis en cascada con trazabilidad de redirecciones aplicable a diversos entornos |  |  |
|  | 2.1. | Matriz de Analisís de Involucrados |  |  |
|  | 2.1.3 | Matriz Poder-Influencia |  |  |
|  |  | N° | Involucrados | Tipos |
|  |  | 1 | Msc. Ángela Yanza Montalván | Directo |
|  |  | 2 | Morán Vera Mickaell Adrián (Autor) | Directo |
|  |  | 3 | Universidad de Guayaquil (Autoridades) | Directo |
|  |  | 4 | Carrera de Software – FCMF | Directo |
|  |  | 5 | Estudiantes de la UG (población de estudio) | Directo |
|  |  | 6 | Usuarios de Guayaquil expuestos al quishing | Indirecto |
|  |  | 7 | Comerciantes y PYMES que usan códigos QR | Indirecto |
|  |  | 8 | Entidades financieras con canales QR | Indirecto |
|  |  | 9 | Futuros proyectos de investigación en ciberseguridad | Indirecto |
|  |  | Influencia |  |  |
|  |  |  | Poder |  |

## Hoja: 2.2.1

| Mapa de Actores |  |
| --- | --- |
|  | Proyecto:  Sistema inteligente para la detección de suplantación en códigos QR mediante análisis en cascada con trazabilidad de redirecciones aplicable a diversos entornos |
| 2.2. | Mapas de Actores |
| 2.2.1 | Mapas de Actores |

## Hoja: 3. AP

|  |  | Metodología de Marco Lógico |  |
| --- | --- | --- | --- |
|  |  | Fase 3. Análisis de Problemas |  |
|  |  |  | Proyecto:     Sistema inteligente para la detección de suplantación en códigos QR mediante análisis en cascada con trazabilidad de redirecciones aplicable a diversos entornos |

## Hoja: 3.1

|  |  | Análisis de Problemas |  |
| --- | --- | --- | --- |
|  |  | Proyecto:    Sistema inteligente para la detección de suplantación en códigos QR mediante análisis en cascada con trazabilidad de redirecciones aplicable a diversos entornos |  |
|  |  | 3. | Analisís de Problemas |
|  |  | 3.1. | Árbol de Problemas |

## Hoja: 4. AO

|  |  |  | Metodología de Marco Lógico |  |
| --- | --- | --- | --- | --- |
|  |  |  | Fase 4. Análisis de Objetivos |  |
|  |  |  |  | Proyecto:      Sistema inteligente para la detección de suplantación en códigos QR mediante análisis en cascada con trazabilidad de redirecciones aplicable a diversos entornos |

## Hoja: 4. 1

|  |  |  | Analisis de Objetivos |
| --- | --- | --- | --- |
|  |  |  | Proyecto:    Sistema inteligente para la detección de suplantación en códigos QR mediante análisis en cascada con trazabilidad de redirecciones aplicable a diversos entornos |
|  |  | 4. | Analisís de Objetivos |
|  |  | 4.1. | Árbol de Objetivos |

## Hoja: 5. AA

|  |  | Metodología de Marco Lógico |  |
| --- | --- | --- | --- |
|  |  | Fase 5. Análisis de Alternativas |  |
|  |  |  | Proyecto:     Sistema inteligente para la detección de suplantación en códigos QR mediante análisis en cascada con trazabilidad de redirecciones aplicable a diversos entornos |

## Hoja: 5.1

|  |  | Análisis de Alternativas |  |
| --- | --- | --- | --- |
|  |  |  | Proyecto:    Sistema inteligente para la detección de suplantación en códigos QR mediante análisis en cascada con trazabilidad de redirecciones aplicable a diversos entornos |
|  |  | 5. | Analisís de Alternativas |
|  |  | 5.1. | Árbol de Alternativas |

## Hoja: 6. DE

|  |  | Metodología de Marco Lógico |  |
| --- | --- | --- | --- |
|  |  | Fase 6. Diseño de Estratégico |  |
|  |  |  | Proyecto:     Sistema inteligente para la detección de suplantación en códigos QR mediante análisis en cascada con trazabilidad de redirecciones aplicable a diversos entornos |

## Hoja: 6.1

|  | Diseño de Estratégico |  |  |
| --- | --- | --- | --- |
|  |  |  | Proyecto:    Sistema inteligente para la detección de suplantación en códigos QR mediante análisis en cascada con trazabilidad de redirecciones aplicable a diversos entornos |
|  |  | 6. | Diseño de Estratégico |
|  |  | 6.1. | Estructura Analítica del Proyecto (EAP) |

## Hoja: 7. MML

|  |  | Metodología de Marco Lógico |  |
| --- | --- | --- | --- |
|  |  | Fase 7. Análisis de Matriz del Marco Lógico |  |
|  |  |  | Proyecto:     Sistema inteligente para la detección de suplantación en códigos QR mediante análisis en cascada con trazabilidad de redirecciones aplicable a diversos entornos |

## Hoja: 7.1

|  |  | Matriz de Marco Lógico |  |  |
| --- | --- | --- | --- | --- |
|  |  |  |  | Proyecto:    Sistema inteligente para la detección de suplantación en códigos QR mediante análisis en cascada con trazabilidad de redirecciones aplicable a diversos entornos |
|  |  | 7. | Análisis de matriz del marco lógico |  |
|  |  | 7.1. | Matriz del Marco Lógico |  |

